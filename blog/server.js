...
mongoose.connect('mongodb://user:user123@ds011870.mlab.com:11870/blogdatabase', function (err) {
    if (err) {
...
router.route('/login').post(function (req, res) {
    if (req.body.isNew) {
        ...
        User.findOne({ login: req.body.login, password: req.body.password}, 'name').exec(function (err, user) {
            if (err) res.send(err);
            if (user != null) {
...
router.route('/posts/:post_id?').get(function (req, res) {
    ...
    if (post.title == null)
      res.status(400).send('Title don`t be null!');
